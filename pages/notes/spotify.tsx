...
if (env == "development") {
  ...
  url = "https://darshanaswath.vercel.app/";
}
...
export default function spotify(props: any) {
  const podcastQuery = useQuery("podcast", getPodcasts, {
    initialData: props.podcast,
  });
  ...
  return (
    <Layout title="Spotify | Darshan Aswath">
      <div className="flex flex-col justify-center items-start max-w-2xl border-gray-200 dark:border-gray-700 mx-auto pb-16 space-y-6">
        <div>
          <h1 className="font-bold text-3xl md:text-4xl pb-2 text-left">
            Podcasts & Music
          </h1>
          <p className="pb-2 font-Quattro text-sm text-gray-600 dark:text-gray-300">
            Curated list of intersting Podcasts and lovely Songs i listen to on
            a daily basis on Spotify, updated daily.
          </p>
          <hr className="w-full border-1 border-gray-200 dark:border-gray-800 my-2 pb-2"></hr>
          <h1 className="font-bold text-lg pb-3 text-left text-gray-600 dark:text-gray-300 pl-3">
            Podcasts
          </h1>
          {podcastQuery.data?.map((podcast) => {
            return (
              <div
                className="hover:bg-gray-100 dark:hover:bg-gray-800 rounded-md p-3 transition ease-in-out duration-200"
                key={podcast.title}
              >
                <FavSpotifyCards
                  image={podcast.albumImageUrl}
                  link={podcast.Url}
                  title={podcast.title}
